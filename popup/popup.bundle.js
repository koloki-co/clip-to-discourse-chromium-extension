...
var DEFAULT_EXCERPT_LENGTH = 800;
...
`;
  ...
  if (tag.startsWith("h") && tag.length === 2) {
    ...
    if (Number.isInteger(level) && level >= 1 && level <= 6) {
      return `
${"#".repeat(level)} ${collapseWhitespace2(renderChildren(node, listDepth))}

`;
    }
  }
  if (tag === "blockquote") {
    const content2 = mergeParagraphs(renderChildren(node, listDepth));
    if (!content2) return "";
    return `
${content2.split("\n").map((line) => `> ${line}`).join("\n")}
...
`;
  ...
  if (tag === "ul" || tag === "ol") {
    const items = Array.from(node.children).filter((child) => child.tagName && child.tagName.toLowerCase() === "li").map((child, index) => {
      const prefix = tag === "ol" ? `${index + 1}.` : "-";
      const content2 = mergeParagraphs(renderChildren(child, listDepth + 1));
      const indent = "  ".repeat(listDepth);
      return `${indent}${prefix} ${content2}`;
    }).join("\n");
    return `
${items}
...
function htmlToMarkdown(html) {
  ...
  if (typeof DOMParser === "undefined") {
    return collapseWhitespace2(html.replace(/<[^>]+>/g, " "));
  }
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, "text/html");
  ...
  return collapseWhitespace2(markdown);
}
var Readability = import_readability.default.Readability || import_readability.default;
var DEFAULT_FULL_TEXT_OPTIONS = {
  includeImages: true,
  includeLinks: true,
  includeTables: true,
...
`;
  ...
  turndown.addRule("removeEmpty", {
    filter: (node) => {
      return ["P", "DIV", "SPAN"].includes(node.nodeName) && (!node.textContent || node.textContent.trim() === "");
    },
    replacement: () => ""
  });
  return turndown;
...
function cleanAttributes(doc) {
  const keepAttributes = /* @__PURE__ */ new Set(["href", "src", "alt", "title", "colspan", "rowspan", "align"]);
  doc.querySelectorAll("*").forEach((node) => {
    [...node.attributes].forEach((attr) => {
      ...
      if (name.startsWith("data-") && ["pre", "code", "div", "figure"].includes(tagName)) {
        return;
      }
...
function removeEmptyElements(doc) {
  const importantTags = /* @__PURE__ */ new Set(["img", "br", "hr", "input", "iframe"]);
  ...
  doc.querySelectorAll("*").forEach((node) => {
    ...
    const hasContentTags = node.querySelectorAll("p, h1, h2, h3, h4, h5, h6, ul, ol, table, blockquote, pre").length > 0;
...
function convertPseudoHeadings(doc) {
  doc.querySelectorAll("div, span").forEach((el) => {
    ...
    if (className.includes("title") || className.includes("heading") || style.includes("font-weight: bold") || style.includes("font-weight:bold")) {
...
function htmlToMarkdownFullPage(html, options = {}) {
  if (!html) {
    return "";
  }
  if (typeof DOMParser === "undefined") {
...
function buildExcerpt(text, maxLength = DEFAULT_EXCERPT_LENGTH) {
  const normalized = collapseWhitespace2(text || "");
  if (!normalized) {
    return "";
  }
