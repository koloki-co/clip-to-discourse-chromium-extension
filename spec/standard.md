...
- Selection override: if a non-whitespace browser text selection exists when the popup opens, "Text Selection" is checked automatically, overriding the initially active profile's default. This happens once on open; it is not re-applied if the user selects another clip style or switches profiles. There is no per-profile opt-out. See [context-menu.md](context-menu.md) ("Automatic Clip Style Selection") for implementation details.
