...
- [x] **R63 - Decide and document credential storage and transport policy** covering plaintext API keys in synced storage and whether to warn on or reject `http://` base URLs.
  ...
  **Decision: Concern 1 - Option B (local storage for credentials), resolved as a side effect of R61 Option B.** Credentials (Admin API keys and User API keys) will live in `chrome.storage.local`, not sync, so they never replicate through the user's Google account. Only documentation is needed in the store listing and SECURITY.md to describe the change.

  Additionally, a **passphrase-protected encryption** option (Option C) will be offered as a future enhancement for the security-conscious user who wants credentials encrypted at rest. This is not the default - it is an opt-in feature with a passphrase UI, key derivation, and an explicit recovery warning. The extension will store a derived key hash (not the passphrase itself) and use it to encrypt/decrypt credential fields before writing to `storage.local`. This is tracked as a separate item after R61 lands.

  **Decision: Concern 2 - `http://` disabled by default with an advanced opt-in checkbox.** Discourse requires HTTPS during installation (SSL is one of the first setup steps), so `http://` is rarely needed. The base URL field will reject `http://` by default. An "Allow HTTP connections (advanced)" checkbox in the options page, accompanied by a warning that credentials will be sent in plaintext, will unlock the field for the rare legitimate case (localhost dev, intranet behind VPN). This is safer than a dismissible warning because the user must take an explicit action to enable the insecure mode.
